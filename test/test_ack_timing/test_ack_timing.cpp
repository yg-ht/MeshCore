...
LocalIdentity::LocalIdentity() = default;
...
public:
  ...
  void setExtraAcks(uint8_t count) { extra_acks = count; }
...
TEST_F(AckTimingTest, FinalAckIsSeparatedFromMultipartCopy) {
  radio.airtime = 10;
  rng.value = 0;
  mesh.setExtraAcks(1);
  ...
  ASSERT_EQ(2u, manager.outbound.size());
  EXPECT_GE(manager.outbound[1].scheduled_for, manager.outbound[0].scheduled_for + radio.airtime + 50);
}
